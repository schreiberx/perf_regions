...
class TestStreamBenchmark(unittest.TestCase):
    ...
    def test_stream_execution(self):
        ...
        self.execute("make build/stream_c_perfregions")

        # Run
        print("Running build/stream_c_perfregions")
        try:
            output = subprocess.check_output(
                ["build/stream_c_perfregions"],
                cwd=self.base_dir,
                env=self.env,
        ...
        try:
            ...
            print(
                f"Headers found: {headers}. Missing expected columns for bandwidth calc."
            )
