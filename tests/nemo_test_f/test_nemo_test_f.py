...
def test_original_execution(base_dir, env, clean_artifact):
    run_command("make build/nemo_test", base_dir, env)
    run_command("taskset -c 0 build/nemo_test", base_dir, env)


@pytest.mark.parametrize("env", ["papi", "wallclock"], indirect=True)
def test_instrumented_execution(base_dir, env, clean_artifact):
    run_command("make build/nemo_test_perf_regions", base_dir, env)
    run_command("taskset -c 0 build/nemo_test_perf_regions", base_dir, env)
...
def test_instrumentation_diffs(base_dir, env, clean_artifact):
    ...
    # Check src untouched
    run_command(
        "diff src/nemo_test.F90 test_data/nemo_test.F90_TEST_ORIG", base_dir, env
    )

    # Check build instrumented
    run_command(
        "diff build/nemo_test.F90 test_data/nemo_test.F90_TEST_PR", base_dir, env
    )
...
def test_cleanup_restoration(base_dir, env, clean_artifact):
    ...
    target = os.path.join(base_dir, "build/nemo_test.F90")

    # Run cleanup
    run_command(f"python3 {script} {target} cleanup", base_dir, env)

    # Diff with ORIG
    run_command(f"diff {target} test_data/nemo_test.F90_TEST_ORIG", base_dir, env)
