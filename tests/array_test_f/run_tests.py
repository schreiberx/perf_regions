...
class TestArrayTestFortran(unittest.TestCase):
    ...
    def test_03_instrumentation_diffs(self):
        ...
        self.execute("diff src/array_test.F90 test_data/array_test.F90_TEST_ORIG")

        # 2. Verify build output MATCHES PR reference
        self.execute("diff build/array_test.F90 test_data/array_test.F90_TEST_PR")

    def test_04_cleanup(self):
        print("\nRunning cleanup test...")
        self.execute("make build/array_test_perf_regions")
        self.execute("make clean")
        self.assertFalse(os.path.exists(os.path.join(self.base_dir, "build/array_test.F90")))
