...
def test_instrumentation_diffs(base_dir, env, clean_artifact):
    ...
    # Check src untouched
    run_command(
        "diff src/array_test.F90 test_data/array_test.F90_TEST_ORIG", base_dir, env
    )

    # Check build instrumented
    run_command(
        "diff build/array_test.F90 test_data/array_test.F90_TEST_PR", base_dir, env
    )
...
def test_cleanup_restoration(base_dir, env, clean_artifact):
    ...
    # 2. Run cleanup script on build/array_test.F90
    script = os.path.join(base_dir, "bin/perf_regions_instrumentation.py")
    target = os.path.join(base_dir, "build/array_test.F90")

    # Need to run python script.
    run_command(f"python3 {script} {target} cleanup", base_dir, env)

    # 3. Diff with ORIG
    run_command(f"diff {target} test_data/array_test.F90_TEST_ORIG", base_dir, env)
