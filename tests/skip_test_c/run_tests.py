...
class TestSkipTest(unittest.TestCase):
    ...
    def test_skip_execution(self):
        ...
        # Build
        self.execute("make build/skip_test")
        ...
        # Run
        subprocess.check_call(["build/skip_test"], cwd=self.base_dir, env=run_env)
        ...
        self.assertEqual(
            outer.get("counter"),
            3,
            f"Outer region count should be 3, got {outer.get('counter')}",
        )
        self.assertEqual(
            outer.get("skipped"),
            2,
            f"Outer region skipped should be 2, got {outer.get('skipped')}",
        )

        # Independent (also loop 5)
        self.assertEqual(
            independent.get("counter"),
            3,
            f"Independent count should be 3, got {independent.get('counter')}",
        )
        self.assertEqual(
            independent.get("skipped"),
            2,
    ...
    def test_no_skip_execution(self):
        ...
        self.assertEqual(
            outer.get("counter"),
            5,
            f"Outer region count should be 5 with SKIP_N=0, got {outer.get('counter')}",
        )
        self.assertEqual(
            outer.get("skipped"),
            0,
            f"Outer region skipped should be 0, got {outer.get('skipped')}",
        )
